      ...
      <div className="flex flex-row items-center space-x-4">
        ...
        <div className="flex flex-col">
          <div className="flex flex-row space-x-2 items-center">
            <span className="text-xs font-bold">{fake.name || 'Jane Doe'}</span>
            <MdVerified
              className={`${fake.isVerified ? 'shown' : 'hidden'} text-primary`}
            />
          </div>
  ...
  return (
    <div className="w-full space-y-6 xl:ml-10">
      <div className="flex flex-row items-center justify-between">
        ...
        <Link href="" passHref>
          <span className="font-medium text-primary text-xs">See All</span>
        </Link>
      </div>
